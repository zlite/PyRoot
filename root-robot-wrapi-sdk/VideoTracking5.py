...
def findWaypoints(imageIn, imageOut, lowerBound, upperBound, centerColor, boundsColor, drawCircle=True):
    # Convert image to HSV format:
    hsv = cv2.cvtColor(imageIn, cv2.COLOR_BGR2HSV)
    ...
    for c in addedContours:
        ...
        if drawCircle:
            cv2.circle(imageOut, center, int(radius), (255, 0, 0), 2)
...
while True:
    ...
    findWaypoints(inputImage, outputImage, redLower, redUpper, pointsContourColor, pointsContourColor, True)
